...
def instrumented_rollout(simulator: FixedRouteSimulator, choose) -> dict:
    ...
    while not simulator.state.completed and steps < max_steps:
        ...
        if not shield.any_legal:
            g += failure_step_reward(simulator, decision_time=simulator.state.time.value)
        ...
        if discrete >= len(shield.mask) or not shield.mask[discrete]:
            g += failure_step_reward(simulator, decision_time=t_before)
...
def main() -> int:
    ...
    payload = {
        "label": "correctness_audit_visited_continuation",
        "test_used": False,
